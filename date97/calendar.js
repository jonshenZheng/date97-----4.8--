...
function My97DP() {
    ...
    this.init();
    /**
     * [focusArr description]
     * @type {Array}
     * todayI 是今天的按钮、okI确定按钮，clearI清除按钮 
     * 绑定的节点在_initPoint这个函数中
     */
...
My97DP.prototype = {
    ...
    splitDate: function(str, fmt, y, M, d, H, m, s, b3x) {
        ...
        if (str && str.loadDate) {
            ...
            dt = new DPDate();
            fmt = fmt || $dp.dateFmt;
            var i, splitStr, offset = 0, match, tokenRe = /yyyy|yyy|yy|y|MMMM|MMM|MM|M|dd|d|%ld|HH|H|mm|m|ss|s|DD|D|WW|W|w/g;
    ...
    _dealFmt: function() {
        ...
        function _setHas(re) {
            var p = (re + "").slice(1, 2);
            $dp.has[p] = re.exec($dp.dateFmt) ? ($dp.has.minUnit = p,
            true) : false
        }
    },
...
function getWeek(dt, offset) {
    offset = offset || 0;
    var d = new Date(dt.y,dt.M - 1,dt.d + offset), weekNum;
...
function day_Click(y, M, d, H, m, s) {
    ...
    if (!callFunc("onpicking")) {
        ...
        if ($c.autoPickDate || isCurrDay || arguments.length == 0) {
            /*pickDate 设置节点日期 */
            $c.pickDate();
            if( typeof $cfg.callbackFn === 'function' ){
                $cfg.callbackFn($dp.el.realValue);
            }
...
function attachTabEvent(o, func) {
    $dp.attachEvent(o, "onkeydown", function(e) {
        e = e || event,
...
function updownEvent(offset) {
    ...
    var i, p = cf == $d.HI ? "H" : (cf == $d.mI ? "m" : "s"), interval = hmsCfg[p][0], v = $dt[p] + offset * interval;
...
function _tab(e) {
    ...
    if ($dp.enableInputMask && !$OPERA && !$dp.readOnly && $c.currFocus == $dp.el && (k >= 48 && k <= 57)) {
        var el = $dp.el, v = el.value, pos = getPosition(el), ss = {
            str: "",
            arr: []
        }, i = 0, mm, vStart = 0, vEnd = 0, n = 0, offset, tokenRe = /yyyy|yyy|yy|y|MM|M|dd|d|%ld|HH|H|mm|m|ss|s|WW|W|w/g, g = $dp.dateFmt.match(tokenRe), t1, t2, t3, t4, t5, vlen, offset = 0;
        if (v != "") {
            n = v.match(/[0-9]/g);
            n = n == null ? 0 : n.length;
            for (i = 0; i < g.length; i++) {
